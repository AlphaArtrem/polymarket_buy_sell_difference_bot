...
from polymarket_arb.reporting.writers import write_catalog_snapshot, write_run_summary
...
def build_live_adapter(
    settings: Settings, catalog: list[MarketCatalogEntry]
) -> LiveAdapter:
...
def record_live(
    ...
    if not catalog:
        typer.echo("No active curated binary markets were resolved from Gamma.", err=True)
        raise typer.Exit(code=1)
    adapter = build_live_adapter(settings, catalog)
